...
- **Application tracker** — one button, one tap per job application. Today / last 7 days / per-day average, and a 14-day bar chart. **view log** opens the full history grouped by day, where you can delete a single mistaken entry or clear a whole day at once. **Undo** (or Ctrl+Z) steps back through every application change — a log, a delete, or a whole-day clear — and the button names what it will reverse. History is per-session, so a reload starts it fresh.
